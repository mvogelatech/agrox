...
import {
	AddressRegistrationDTO,
	FarmRegistrationDTO,
	UserRegistrationDTO,
	FieldRegistrationDTO,
	AreaRegistrationDTO,
	CropRegistrationDTO,
	RegistrationDTO,
...
function generateRandomUniquePassword(currentPasswords: string[]) {
	for (let i = 0; i < 1000; i++) {
		const generatedPassword = Math.floor(Math.random() * 10000).toFixed(0);
		if (!currentPasswords.includes(generatedPassword)) return generatedPassword;
	}
...
export class FarmService {
	...
	async registerFarmBulk(data: RegistrationDTO) {
		...
		// check if address/farm already exists
					...
					data: {
						...
						crop: {
							create: {
								...
								sowing_date: parse(afcData.cropData.sowingDate, 'dd/MM/yyyy', new Date()),
								expected_harvest_date: parse(afcData.cropData.expectedHarvestDate, 'dd/MM/yyyy', new Date()),
	...
	async registerFarm(farmData: FarmRegistrationDTO, addressData: AddressRegistrationDTO) {
		const state = await prisma.state.findUnique({
			where: {
	...
	async registerUser(userData: UserRegistrationDTO, farmId: number) {
		const currentPasswords = await prisma.user.findMany({ select: { password: true } });
		const newPassword = generateRandomUniquePassword(currentPasswords.map(({ password }) => password));
	...
	async registerField(areaData: AreaRegistrationDTO, fieldData: FieldRegistrationDTO, cropData: CropRegistrationDTO, famrId: number) {
		// check if there is an area
		let area = await prisma.area.findUnique({
			where: {
				name_per_farm_un: {
		...
		if (cropData.cropType !== '' && cropData.variety !== '' && cropData.sowingDate !== '' && cropData.expectedHarvestDate !== '') {
			const crop = await prisma.crop.create({
				data: {
					...
					sowing_date: parse(cropData.sowingDate, 'dd/MM/yyyy', new Date()),
					expected_harvest_date: parse(cropData.expectedHarvestDate, 'dd/MM/yyyy', new Date()),
