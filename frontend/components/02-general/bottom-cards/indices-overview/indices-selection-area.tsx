...
import { cacheAreaIndexImages, getDatesForIndexDataFromArea } from '../../../../src/utils/visiona-indices';

type MySliderRef = (React.LegacyRef<Slider> & React.MutableRefObject<SliderRef>) | undefined;

export function IndicesSelectionArea(props: CloseableBottomCardProps) {
	...
	const [indexDates, setIndexDates] = useState(getDatesForIndexDataFromArea(area));
	...
	const onChange = (event: Event, selectedDate: Date | undefined) => {
		...
		if (selectedDate) {
			...
			const newDates = getDatesForIndexDataFromArea(area, selectedDate.toISOString());
	...
	const onDateSliderChange = (value: number) => {
		const selectedDate = indexDates[value];
		dispatch({ type: 'CHANGE_INDICES_DATE', indexDate: selectedDate });
	};

	useMemo(() => {
		(async () => {
			setShowIndicator(true);
			await cacheAreaIndexImages(area, indicesTab);
	...
	}, [area, indicesTab, indexDates]);

	return (
		<View style={styles.mainView}>
			{showIndicator ? <WaitOverlay text="Esta operação é demorada..." /> : undefined}
